...
def wait_scan(client: httpx.Client, scan_id: int, timeout: float = 120.0) -> dict:
    deadline = time.time() + timeout
    last = {}
    while time.time() < deadline:
        try:
...
def start_scan(client: httpx.Client, payload: dict) -> int:
...
def main() -> int:
    ...
    try:
        ...
        with httpx.Client(base_url=BACKEND_URL, timeout=120.0) as client:
            client.post("/api/lab/scenario", json={"state": "VULNERABLE"}).raise_for_status()
            manifest = client.get("/api/lab/manifest")
            manifest.raise_for_status()
            assert len(manifest.json()["surfaces"]) >= 10, "lab surfaces missing"

            mapped = client.post("/api/active/map", json={"target_url": TARGET, "selected_modules": ["xss", "access_control", "csrf"]})
            ...
            defend_id = start_scan(client, {"target_url": TARGET, "mode": "defend", "intensity": "low"})
            defend = wait_scan(client, defend_id)
            assert defend["status"] == "complete", defend
            defend_artifacts = client.get(f"/api/scan/{defend_id}/artifacts").json()
            ...
            pentest_id = start_scan(client, {"target_url": TARGET, "mode": "pentest", "intensity": "low", "selected_tests": modules})
            pentest = wait_scan(client, pentest_id)
            assert pentest["status"] == "complete", pentest
            pentest_artifacts = client.get(f"/api/scan/{pentest_id}/artifacts").json()
            ...
            persisted_findings = client.get("/api/findings", params={"scan_id": pentest_id}).json()
            verify_candidate = next((item for item in persisted_findings if item.get("module") == "xss"), persisted_findings[0])
            client.post("/api/lab/scenario", json={"state": "PATCHED"}).raise_for_status()
            verify = client.post(f"/api/findings/{verify_candidate['id']}/verify")
            verify.raise_for_status()
            assert verify.json()["status"] == "FIX_VERIFIED", verify.text
            print("E2E PASS verify fix")

            second_id = start_scan(client, {"target_url": TARGET, "mode": "pentest", "intensity": "low", "selected_tests": ["xss"]})
            second = wait_scan(client, second_id)
            assert second["status"] == "complete", second
            comparison = client.get(f"/api/ai/scan/{second_id}/analysis").json().get("scan_comparison", {})
            assert comparison.get("previous_scan_id") is not None, comparison
            print("E2E PASS scan comparison")

            client.post("/api/lab/scenario", json={"state": "VULNERABLE"}).raise_for_status()
            stop_id = start_scan(client, {"target_url": TARGET, "mode": "pentest", "intensity": "low", "selected_tests": ["xss", "access_control", "csrf", "security_headers", "sensitive_exposure"]})
            stop_response = client.post(f"/api/scan/{stop_id}/stop")
            assert stop_response.status_code in {200, 409}, stop_response.text
            stopped = wait_scan(client, stop_id, timeout=30.0)
