...
class TestHindiExplainer(unittest.TestCase):
    def test_hindi_explanation_contains_both_languages(self):
        explanation = self._mock_hindi_explanation("Missing Content Security Policy header")
        self.assertIn("English", explanation)
        self.assertIn("Hindi", explanation)

    def _mock_hindi_explanation(self, vuln_name: str) -> str:
        return (
            "English: Missing Content Security Policy header allows XSS attacks. "
            "Fix: Add CSP header to your server configuration.\n\n"
            "Hindi: CSP header missing hai. Iski wajah se attacker aapke website par "
            "malicious script daal sakta hai. Isko theek karne ke liye server mein CSP header add karein."
        )
