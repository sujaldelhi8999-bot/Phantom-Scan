...
function FindingDrawer({ finding, onClose }: { finding: Finding | null; onClose: () => void }) {
  ...
  const hindiRecord = finding
    ...
    : null;
  const hindiText = hindiRecord ? [hindiRecord.how_exploited, hindiRecord.fix, hindiRecord.recommendation].filter(Boolean).join('\n\n') : '';
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between gap-3 mb-1.5">
              <h3 className="text-xs font-semibold text-[var(--text-strong)]">AI Explanation</h3>
              <div className="flex rounded-lg bg-[var(--surface-tertiary)] p-0.5">
                <button onClick={() => setLanguage('en')} className={`rounded-md px-2 py-0.5 text-[10px] font-medium ${language === 'en' ? 'bg-white text-[var(--brand)] shadow-sm' : 'text-[var(--text-muted)]'}`}>EN</button>
                <button onClick={() => setLanguage('hi')} className={`rounded-md px-2 py-0.5 text-[10px] font-medium ${language === 'hi' ? 'bg-white text-[var(--brand)] shadow-sm' : 'text-[var(--text-muted)]'}`}>HI</button>
              </div>
            </div>
