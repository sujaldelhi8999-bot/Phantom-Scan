      ...
      <main>
        <section className="mx-auto grid max-w-6xl gap-8 px-5 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:py-20">
          <div>
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-[var(--text-muted)]">
              PhantomScan helps developers and small teams run vulnerability scans, understand findings, and ship safer software without a heavy security stack.
            </p>
          ...
          <div className="rounded-2xl border border-[var(--border-light)] bg-white p-4 shadow-[var(--shadow-float)]">
            <div className="rounded-xl bg-[var(--surface-secondary)] p-4">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-[11px] text-[var(--text-muted)]">Everything security in one place</div>
                </div>
                <span className="rounded-full bg-[var(--success-soft)] px-2 py-1 text-[10px] font-semibold text-[var(--success)]">Online</span>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {workspaceItems.map((item, index) => (
                  <div key={item} className={cx('rounded-lg border border-[var(--border-light)] bg-white p-3', index === 2 && 'sm:col-span-2')}>
        ...
        <section className="mx-auto grid max-w-6xl gap-4 px-5 pb-16 md:grid-cols-3">
          {features.map((feature) => {
            ...
            return (
              <div key={feature.title} className="rounded-xl border border-[var(--border-light)] bg-white p-5 shadow-[var(--shadow-card)]">
                <div className="mb-4 flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--brand-soft)]">
                  <Icon className="h-4 w-4 text-[var(--brand)]" />
                </div>
                <h2 className="text-sm font-semibold text-[var(--text-strong)]">{feature.title}</h2>
